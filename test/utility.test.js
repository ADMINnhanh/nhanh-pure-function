import { _Utility_MergeObjects } from "../src/Utility";

describe("_Utility_MergeObjects", () => {
  it("should return B when types of A and B are different", () => {
    const A = { key: "value" };
    const B = [1, 2, 3];
    expect(_Utility_MergeObjects(A, B)).toBe(B);
  });
  ...
  it("should merge objects with overlapping keys", () => {
    ...
    expect(_Utility_MergeObjects(A, B)).toEqual(expected);
  });
  ...
  it("should handle empty objects", () => {
    const A = {};
    const B = {};
    expect(_Utility_MergeObjects(A, B)).toEqual({});
  });
